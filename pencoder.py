...
class PositionEmbeddingLearnedWithPoseToken(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        p_emb = torch.cat([self.pose_token_embed(
            p), self.pose_token_embed(p)]).repeat(x.shape[0], 1)
...
def build_position_encoding(config):
    ...
    learn_embedding_with_pose_token = config.get(
        "learn_embedding_with_pose_token")
